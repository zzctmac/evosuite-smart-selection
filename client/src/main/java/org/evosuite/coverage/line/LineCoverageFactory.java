...
public class LineCoverageFactory extends
    ...
    public List<LineCoverageTestFitness> getCoverageGoalsWithSmartCombineForSuite(LineCoverageSuiteFitness lineCoverageSuiteFitness) {
        List<LineCoverageTestFitness> goals = getCoverageGoals();
        if(Utils.enableSmartCombine()) {
            List<LineCoverageTestFitness> removeGoals = new ArrayList<>();
            removeBySmartCombine(goals, removeGoals);
            int originSize = goals.size();
            goals.removeAll(removeGoals);
            if(lineCoverageSuiteFitness != null)
                SuiteGoalRecover.recordLineSelected(goals, lineCoverageSuiteFitness, originSize);
        }
        return goals;
    }

    public Map<Integer, List<LineCoverageTestFitness>> getCoverageGoalsWithSmartCombineForMOSA() {
        List<LineCoverageTestFitness> goals = getCoverageGoals();
        List<LineCoverageTestFitness> removeGoals = new ArrayList<>();
        removeBySmartCombine(goals, removeGoals);
        goals.removeAll(removeGoals);
        Map<Integer, List<LineCoverageTestFitness>> ret = new HashMap<>();
        ret.put(0, goals);
        ret.put(1, removeGoals);
        return ret;
    }

    public static void removeBySmartCombine(List<LineCoverageTestFitness> goals, List<LineCoverageTestFitness> removeGoals) {
        for (LineCoverageTestFitness goal : goals) {
            int span = goal.getLine() - goal.goalInstruction.getBasicBlock().getFirstLine();
            if(goal.getLine() == goal.goalInstruction.getBasicBlock().getLastLine() && span >= Utils.getLineSpan()) {
                continue;
            }
            removeGoals.add(goal);
        }
        LoggingUtils.getEvoLogger().warn(String.format("remove %d lines by SmartCombine(total %d lines)", removeGoals.size(), goals.size()));
    }
