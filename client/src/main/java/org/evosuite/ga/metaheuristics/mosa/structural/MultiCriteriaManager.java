...
import org.evosuite.smartcombine.MainObjectiveCriteriaSelector;
...
public class MultiCriteriaManager extends StructuralGoalManager implements Serializable {
    ...
    protected Set<TestFitnessFunction> focusedGoal;
    ...
    public MultiCriteriaManager(List<TestFitnessFunction> targets) {
        super(targets);
        focusedGoal = new HashSet<>(targets);
    ...
    private void addDependencies4Methods() {
        ...
        if(Utils.enableSmartCombine() && MainObjectiveCriteriaSelector.penaltyObjectives.contains(Criterion.METHOD)) {
            return;
        }
    ...
    private void addDependencies4MethodsNoException() {
        ...
        if(Utils.enableSmartCombine() && MainObjectiveCriteriaSelector.penaltyObjectives.contains(Criterion.METHODNOEXCEPTION)) {
            return;
        }
        for (BranchCoverageTestFitness branch : this.dependencies.keySet()) {
